...
if (isset($_REQUEST['password'])  && isset($_REQUEST['passwordConfirm'])  && $_REQUEST['password'] !== "" && $_REQUEST['passwordConfirm'] !== "") {
    if (isset($_REQUEST['user_name'])  && $_REQUEST['user_name'] !== "") {
        ...
        if ($result->num_rows > 0) {
            ...
            //username doesn't exist, so add the user registration request into the db
            ...
            if ($password == $passwordConfirm) {
                ...
                // prepare and bind
                $stmt = $conn->prepare("INSERT INTO tbl_user_pending (firstname, lastname, username, email, password, phone_number, date_of_birth, sex) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                ...
                $param_Status = 'Pending'; //Set the user status to Pending
